...
//            MarCAN_HardReset();
    ...
    #region ECAN20Converter
    public class ECAN20Converter : IUCANConverter
    {
        public event MyDelegate ErrEvent;
        public event MyDelegate Progress;

        #region Всякая шняга
        enum CAN_ChannelBaudRate
        {
	        CAN_BR_1000k, 
	        CAN_BR_800k, 
	        CAN_BR_500k, 
        ...
        // Структура универсального сообщения CAN
        public unsafe struct TUniCanMessage
        {
	        UInt32 messageID;
	        fixed Byte data[8];
	        Byte length;
	        //служебные идентификаторы
	        Byte RTR;		//признак RTR
	        Byte EFF;		//признак расширенного идентификатора (29 бит)
	        UInt32 time;
        };// END STRUCTURE DEFINITION TCanOlsMessage



        //интерфейс обратного вызова для уведомления о принятии команды
        public struct ICanCallback
        {
	        static void DataReceived();	//передача структуры 
        };

        //интерфейс объекта-фильтра сообщений
        public unsafe struct ICanMessageFilter
        {
	        bool AcceptMessage(ref TUniCanMessage msg);
        };
        public struct ICanModuleOLS
        {
	        bool OpenBoard(int chNum, CAN_ChannelBaudRate cbr, bool aBlockMode);	//aBlockMode - работа в блокирующем режиме
	        void CloseBoard();
	        void SetCallback(ref ICanCallback pCallback);
	        void SetFilter(ref ICanMessageFilter pFilter);
	        bool GetMessage(ref TUniCanMessage msg);
	        bool SendMessage(ref TUniCanMessage msg);
	        UInt32 GetStatus();
	        bool IsOpen();
	        void DeleteObject();
        };// END CLASS DEFINITION ICanModuleOLS
        ...
        */
        ...
        [DllImport(@"ElcusCanCnv.dll", CallingConvention = CallingConvention.StdCall)]
        public unsafe static extern ICanModuleOLS* CreateCanCnvObj();
        #endregion
